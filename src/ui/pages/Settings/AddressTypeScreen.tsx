...
import { ADDRESS_TYPES, NET_WORK } from '@/shared/constant';
import CHeader from '@/ui/components/CHeader';
import { FooterBackButton } from '@/ui/components/FooterBackButton';
import { useExtensionIsInTab } from '@/ui/features/browser/tabs';
import { useCurrentAccount } from '@/ui/state/accounts/hooks';
import { useAddressType, useChangeAddressTypeCallback } from '@/ui/state/settings/hooks';
      ...
      <Content style={{ backgroundColor: '#1C1919' }}>
        <div className="flex flex-col items-strech mt-5 gap-3_75 justify-evenly mx-5">
          ...
          {ADDRESS_TYPES.map((item, index) => {
            const displayAddress = publicKeyToAddress(currentAccount.address, item.value, NET_WORK);
            return (
              <Button
                key={index}
                size="large"
