...
import { Layout, Content, Icon, Header, Text, Row, Column } from '@/ui/components';
...
export function AmountItem({ item, inDetail }: { item: ExtraItem, inDetail?: boolean }) {
  ...
  return <Row gap={'sm'} style={{ flexWrap: 'wrap' }} justifyEnd={!inDetail} justifyCenter={inDetail} itemsCenter>
    {
    ...
    {
      item.type === 'RUNES' && <>
        <Tooltip title={item.ticker} overlayStyle={{
    ...
    <Text color={'textDim'} text={item.symbol || item.ticker} ellipsis style={{ maxWidth: inDetail ? undefined : 200 }}
          size={inDetail ? 'md' : 'xs'} />
    {
  ...
  return (
    <Column key={index} style={{ position: 'relative' }} gap={'zero'}>
      <Text text={group.date} color="textDim"
            style={{ position: 'sticky', top: 0, backgroundColor: '#1c1919', marginLeft: 16 }}
            py={'md'} />
      <Column gap={'zero'}>
        {group.historyItems.map((item, index) => {
          ...
          return (
            <Row
              clickable
              justifyBetween full key={`item_${index}`}
              style={{
                  ...
                  isReceived ?
                    <svg xmlns="http://www.w3.org/2000/svg" width="36" height="36" viewBox="0 0 36 36" fill="none">
                      ...
                      <path
                        ...
                        fill="#7CDB98" />
                      <defs>
                        <linearGradient id="paint0_linear_5181_4125" x1="30.75" y1="2.625" x2="3" y2="31.125"
                    ...
                    <svg width="36" height="36" viewBox="0 0 36 36" fill="none" xmlns="http://www.w3.org/2000/svg">
                      ...
                      <path
                        ...
                        fill="#FF8474" />
                      <defs>
                        <linearGradient id="paint0_linear_5181_4101" x1="30.75" y1="2.625" x2="3" y2="31.125"
                                        gradientUnits="userSpaceOnUse">
...
export default function HistoryScreen() {
  ...
  useEffect(() => {
    if (address) {
      ...
      setHistoryGroups([]);
      wallet.getAddressHistory({ address, start: (page - 1) * pageSize, limit: pageSize }).then((res) => {
  ...
  return (<>
      ...
      {
        detail && <HistoryDetail detail={detail} close={() => {
          setDetail(undefined);
        }} />
      }
    </>
