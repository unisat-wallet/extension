...
function SignTxDetails({ txInfo, type, rawTxInfo, runesPriceMap, brc20PriceMap }: {
  txInfo: TxInfo; rawTxInfo?: RawTxInfo; type: TxType
...
export default function SignPsbt({
  ...
  useEffect(() => {
    if (txInfo?.decodedPsbt?.inputInfos) {
      ...
      if (Object.keys(runesMap).length > 0) {
        wallet.getRunesPrice(Object.keys(runesMap)).then(setRunesPriceMap).catch(e => tools.toastError((e as Error).message));
      }
      if (Object.keys(brc20Map).length > 0) {
        wallet.getBrc20sPrice(Object.keys(brc20Map)).then(setBrc20PriceMap).catch(e => tools.toastError((e as Error).message));
      }
    }
  }, [txInfo]);
  ...
  const detailsComponent = useMemo(() => {
    return <SignTxDetails txInfo={txInfo} rawTxInfo={rawTxInfo} type={type} runesPriceMap={runesPriceMap}
