...
import { formatNumber, satoshisToAmount, useApproval, useLocationState, useWallet } from '@/ui/utils';
...
export default function InscribeSend({ params: { data, session } }: Props) {
...
export function InscribeSendScreen() {
  const { ticker, tokenID, protocol, inscriptionNumber } = useLocationState<LocationState>();
...
function InscribeTransferStep({ contextData, updateContextData }: StepProps) {
  ...
  useEffect(() => {
    fetchUtxos();
    wallet
      .getORC20Summary(account.address, contextData.inscriptionNumber, contextData.protocol)
      .then((v) => {
  ...
  return (
    <Layout>
      ...
      <Content>
        <Column full>
          <Column gap="lg" full>
            <Text text="Inscribe SEND" preset="title-bold" textCenter my="lg" />
            ...
            <Column>
              <Row justifyBetween>
                ...
                {tokenBalance ? (
                  <Column>
                    <Text
                      text={`${formatNumber(tokenBalance.availableBalance)} ${tokenBalance.ticker}`}
                      textCenter
                      size="xs"
                      onClick={() => {
  ...
  return (
    <Layout>
      ...
      <Content>
        <Column full>
          <Column gap="lg" full>
            <Text text="Inscribe SEND" preset="title-bold" textCenter mt="lg" />
            ...
            <Column justifyCenter style={{ height: 250 }}>
              ...
              <Column mt="xxl">
                ...
                <Card preset="style2">
                  <Text
                    text={`{"p": "${contextData.protocol}","op":"send","tick":"${tokenBalance.ticker}","id":"${tokenBalance.tokenID}","amt":"${amount}"}`}
                    size="xs"
                  />
                </Card>
              </Column>
...
function InscribeResultStep({
  ...
  const checkResult = async () => {
    const result = await wallet.getORC20InscribeResult(order.orderId);
    if (result.orderOrderStatus !== 2) {
      setTimeout(() => {
        checkResult();
      }, 2000);
      return;
